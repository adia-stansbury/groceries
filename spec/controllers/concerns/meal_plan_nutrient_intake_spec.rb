...
shared_examples_for 'meal_plan_nutrient_intake' do
  ...
  it 'calculates nutrient intake for meal plan with multiple recipes' do
    consumer
    location
    meal_plan
    unit
    ...
    results = MealPlan.nutrient_intake(meal_plan.id)
    total_zinc = results.first['amt_consumed']
    
    expect(total_zinc).to eq('6.442')
  end 

  it 'calculates nutrient intake for duplicates of recipe' do
    consumer
    location
    meal_plan
    unit
